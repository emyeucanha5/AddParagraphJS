...
function randPara(){
	return Math.round(text.length * Math.random()) -1;
}
form.addEventListener('submit', function(e) {
	e.preventDefault();
	const numberOfParagraph = parseInt(number.value);
	if(  numberOfParagraph === 0 ){
		paragraph.innerHTML = ``;
	}
	else if(!numberOfParagraph||numberOfParagraph < 0 ||numberOfParagraph >= 10){
		const randParagraph = randPara();
		paragraph.innerHTML = `<p class="result">${text[randParagraph]}</p>`
	}
	else{
	const textTemp = text.slice(0, numberOfParagraph);

	let para = textTemp.map((elem)=> {
